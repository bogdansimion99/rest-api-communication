...
char *compute_get_request(char *host, char *url, char *query_params,
                            char **cookies, int cookies_count, char** tokens,
                            int tokens_count)
{
    char *message = calloc(BUFLEN, sizeof(char));
    char *line = calloc(LINELEN, sizeof(char));
    char *cookie_buffer = calloc(LINELEN, sizeof(char));
    char *token_buffer = calloc(LINELEN, sizeof(char));
    ...
    if (cookies != NULL) {
        ...
        sprintf(line, "Cookie: %s", cookie_buffer);
        compute_message(message, line);
    }
    ...
    if (tokens != NULL) {
        for (int i = 0; i < tokens_count; i++) {
            if (i == tokens_count - 1) {
                strcat(token_buffer, tokens[i]);
                break;
            }
            strcat(token_buffer, tokens[i]);
            strcat(token_buffer, "; ");
        }
        sprintf(line, "Authorization: Bearer %s", token_buffer);
        compute_message(message, line);
    }
...
char* compute_delete_request (char *host, char *url, char **cookies, 
    int cookies_count, char** tokens, int tokens_count) {
    char *message = calloc(BUFLEN, sizeof(char));
    char *line = calloc(LINELEN, sizeof(char));
    char *body_data_buffer = calloc(LINELEN, sizeof(char));
    char *cookie_buffer = calloc(LINELEN, sizeof(char));
